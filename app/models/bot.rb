  ...
  def self.retweet
    ...
    CLIENT.search("#hailstate", result_type: "recent", since_id: maximum(:retweets_id)).take(5).each do |tweet|
      ...
      if (tweet.in_reply_to_status_id? == false)
        if (tweet_text.include? "mike leach") || (tweet_text.include? "airraid") || (tweet_text.include? "arkansas")
          || (tweet_text.include? "razorbacks") || (tweet_text.include? "costello") || (tweet_text.include? "davis wade") 
